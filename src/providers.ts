export const providers = {
  matic_wildhorn:
    "https://still-patient-hill.matic.quiknode.pro/b4ecf45eb108716acdc775404e5b6bb9a46c18d8/",
  matic_rio:
    "https://nd-435-704-707.p2pify.com/9ad58676aefc2aea283ca0a66724bea6",
  xdai_mainnet: [
    `https://primary.gnosis-chain.rpc.hoprtech.net`,
    `https://secondary.gnosis-chain.rpc.hoprtech.net`,
  ],
  xdai_mainnet_old:
    "https://:39b78eea48bd48c2b687e9e7342bec1e@poa-xdai-archival.gateway.pokt.network/v1/lb/61dbeff75a6d11003821698f",
  xdai_mainnet_old_retired:
    "https://:842d068fca3c53e26c85c5f4eb49e1fc@poa-xdai.gateway.pokt.network/v1/lb/61113fe218656e00369bdb9b",
  eth_gnosis_mainnet: "https://eth-erigon.lsotech.net/",
  eth_gnosis_rinkeby:
    "https://sparkling-spring-meadow.rinkeby.quiknode.pro/da2c0f72384c7fc99b857fca44dec8d06206a1dd/",
  eth_gnosis_mainnet_infura:
    "https://mainnet.infura.io/v3/7f8df481afe542f99fe5e01de8822202",
  eth_gnosis_mainnet_alt: "https://eth-erigon.lsotech.net/",
  eth_goerli: `https://apis.ankr.com/${GOERLI_STAGING_ANKR_API_PROJECT_ID}/${GOERLI_STAGING_ANKR_API_TOKEN}/eth/fast/goerli`,
  eth_ceramic_mainnet: "https://eth-erigon.lsotech.net/",
  eth_ceramic_testnet:
    "https://ropsten.infura.io/v3/bdd0361db7d24b2aa3fdae4d7b7c78fb",
  eth_ceramic_mainnet_infura:
    "https://mainnet.infura.io/v3/bdd0361db7d24b2aa3fdae4d7b7c78fb",
  eth_ceramic_testnet_infura:
    "https://ropsten.infura.io/v3/bdd0361db7d24b2aa3fdae4d7b7c78fb",
};
